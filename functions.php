...
add_image_size( 'project', 760, 1038, true);
add_image_size( 'single', 893, 0, true);
...
function peepsakes_register_my_menus() 
{
    register_nav_menus
    (
        array( 'header-menu' => 'Главное меню', 'footer-menu' => 'Подвал')
    );
}
...
function table_wrapper($content) {
    // match any iframes
    $content = str_replace('<table', '<div class="table_wrap"><table', $content);
    $content = str_replace('</table>', '</table></div>', $content);

    return $content;    
}
add_filter('the_content', 'table_wrapper');
...
function register_styles()
{
  ...
  /* Choices */
  /*wp_enqueue_style('choices', get_template_directory_uri() . '/build/styles/choices.min.css', array('main'), null);
  wp_enqueue_style('style', get_template_directory_uri() . '/build/styles/style.css', array(''), null);*/

  /* Custom */
  //wp_enqueue_style('manyboats', get_template_directory_uri() . '/assets/css/custom.css', array('choices'), '1.0');  

 /* wp_enqueue_script( 'accordion', get_template_directory_uri() . '/assets/js/accordion.js', array('jquery'), true );
  wp_enqueue_script( 'choices', get_template_directory_uri() . '/assets/js/choices.min.js', array('accordion'), true );
  wp_enqueue_script( 'tiny-slider', 'https://cdnjs.cloudflare.com/ajax/libs/tiny-slider/2.9.2/min/tiny-slider.js', array('choices'), true );
  wp_enqueue_script( 'slider', get_template_directory_uri() . '/assets/js/slider.js', array('tiny-slider'), true );
  wp_enqueue_script( 'input-range', get_template_directory_uri() . '/assets/js/input-range.js', array('slider'), true );
  wp_enqueue_script( 'menu', get_template_directory_uri() . '/assets/js/menu.js', array('input-range'), true );
  wp_enqueue_script( 'selected', get_template_directory_uri() . '/assets/js/selected.js', array('menu'), true );
  wp_enqueue_script( 'fixed-product', get_template_directory_uri() . '/assets/js/fixed-product.js', array('selected'), true );*/
}
...
function custom_name($email){ return 'AppData'; }
...
function my_action_form() {   
    ...
    if($_POST['company'])
    {
        $body .= "Компания: ".$_POST['company']." \n\n";   
    }  

    if($_POST['what'])
    {
        $body .= "Интересует: ".$_POST['what']." \n\n";   
    }  

    if($_POST['post'])
    {
        $body .= "Должность: ".$_POST['post']." \n\n";   
    }  

    if($_POST['text'])
    {
        $body .= "Комментарий: ".$_POST['text']." \n\n";   
    }  

    $emailTo = get_option("admin_email");  
   
    $subject = 'AppData '.$title;   
